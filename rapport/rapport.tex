...
% Retrouver la ligne qui récupère le mot de passe et intercaler écriture dans
% un fichier au nom un peu planqué.
% Trouver le moment où le système de fichiers réel est monté avec le déchiffrement
% à la volée => dans init
% Passage de RO à RW
% Utilisation du rc.local pour exécution d'un script arbitraire en root
% Envoi du mot de passe sur internet

% Infection en full shell : + et - (compromis light/expressif, utilisation de sed)
% Problématiques de robustesse

% Script d'envoi très basique, pourrait largement être amélioré.
